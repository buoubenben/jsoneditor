...
// Input too short
if(a.offsetHeight<a.scrollHeight)for(b=0;a.offsetHeight<a.scrollHeight+3&&!(b>100);)b++,c++,a.style.height=c+"px";else{for(b=0;a.offsetHeight>=a.scrollHeight+3&&!(b>100);)b++,c--,a.style.height=c+"px";a.style.height=c+1+"px"}}},this.input.addEventListener("keyup",function(b){a.adjust_height(this)}),this.input.addEventListener("change",function(b){a.adjust_height(this)}),this.adjust_height()),this.format&&this.input.setAttribute("data-schemaformat",this.format),this.control=this.theme.getFormControlB3(this.label,this.input,this.description,this),this.container.appendChild(this.control),
...
this.schema.disable?(this.input.disabled=!0,a.style.color="#c5c5c5",this.control.firstElementChild.checked=!1):this.control.firstElementChild.checked=!0},enable:function(){this.always_disabled||(this.input.disabled=!1,
// TODO: WYSIWYG and Markdown editors
this.label&&(this.label.style.color=""),this.value=this.input.value),this.refreshValue(),this.onChange(!0),this._super()},disable:function(){this.input.disabled=!0,
// TODO: WYSIWYG and Markdown editors
this.label&&(this.label.style.color="#c5c5c5"),this.value=null,
...
var a,b,c;a=document.createElement("div"),a.className+=" widget";for(var d=this.theme.GenNonDuplicateID(),e=this.schema.options.custom_option,f=0;f<e.enum_title.length;f++){var g=this.theme.GenNonDuplicateID();b=document.createElement("label"),b.setAttribute("for","radio"+g),b.innerHTML=e.enum_title[f],c=document.createElement("input"),c.setAttribute("type","radio"),c.name="radio"+d,c.id="radio"+g,c.value=e["enum"][f],a.appendChild(b),a.appendChild(c)}return a},setupCheckboxRadio:function(a){var b=c({},f.plugins.checkboxradio);this.schema.options&&this.schema.options.custom_option&&(b=c(b,this.schema.options.custom_option)),this.checkboxradio=window.jQuery(".widget input").checkboxradio(b),this.initstatus(a)},initstatus:function(a){this.schema.disable?(window.jQuery(this.input).children("input").checkboxradio("disable"),a.style.color="#c5c5c5"):"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},checkListener:function(){//创建监听checkbox
var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})},enable:function(){window.jQuery(this.input).children("input").checkboxradio("enable"),this.label.style.color="",this.value=this.input.value,this.refreshValue(),this.onChange(!0),this._super()},disable:function(){window.jQuery(this.input).children("input").checkboxradio("disable"),this.label.style.color="#c5c5c5",this.input.value=this.value,this.value=null,this.is_dirty=!0,this.onChange(!0),this._super()}}),/**
...
//监听并赋值
this.checkListener()},draw:function(a){var b;return b=document.createElement("input"),b.value=this.schema["default"],b.id=a,b.style.display="none",b.className="jpicker",b},setupJpicker:function(a,b){var d=this,e=function(a,b){var c=a.val("all");d.value="#"+c.ahex,d.refreshValue(),d.onChange(!0)},g=this.schema.options.custom_option,h=c({},f.plugins.jpicker);this.schema.options&&this.schema.options.custom_option&&(h=c(h,g)),window.jQuery.fn.jPicker.defaults.images.clientPath="../images/",this.colorPicker=window.jQuery("#"+b).jPicker(h,e),this.initstatus(a)},initstatus:function(a){this.schema.disable?
// window.jQuery(this.checkboxradio).checkboxradio("disable");
a.style.color="#c5c5c5":"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},checkListener:function(){var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})},enable:function(){this.label.style.color="",
// this.value=window.jQuery.jPicker.List[0].color.active.val('hex');
this.value=this.input.value;
//去掉遮罩层
var a=this.label.nextSibling.nextSibling.firstChild,b=a.getElementsByTagName("div")[0];a.removeChild(b),this.refreshValue(),this.onChange(!0),this._super()},disable:function(){this.label.style.color="#c5c5c5",this.input.value=this.value,this.value=null;
//添加遮罩层
...
c.indexOf(k)!==-1?(this.input.value=k,this.value=k):(this.input.value=c[0],this.value=c[0]||"",this.parent?this.parent.onChildEditorChange(this):this.jsoneditor.onChange(),this.jsoneditor.notifyWatchers(this.path)),this.setupSelect2()}this._super()},enable:function(){this.always_disabled||(this.input.disabled=!1,this.label&&(this.label.style.color=""),this.value=this.input.value,this.select2&&this.select2.select2("enable",!0)),this.refreshValue(),this.onChange(!0),this._super()},disable:function(){this.input.disabled=!0,this.value=null,this.label&&(this.label.style.color="#c5c5c5"),this.select2&&this.select2.select2("enable",!1),this.refreshValue(),this.onChange(!0),this._super()},destroy:function(){this.label&&this.label.parentNode&&this.label.parentNode.removeChild(this.label),this.description&&this.description.parentNode&&this.description.parentNode.removeChild(this.description),this.input&&this.input.parentNode&&this.input.parentNode.removeChild(this.input),this.select2&&(this.select2.select2("destroy"),this.select2=null),this._super()},initstatus:function(a){//初始化状态 是否禁用
this.schema.disable?(this.input.disabled=!0,a.style.color="#c5c5c5","checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!1)):"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},
//checkbox点击监听
checkListener:function(){var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})}}),f.defaults.editors.selectize=f.AbstractEditor.extend({setValue:function(a,b){a=this.typecast(a||"");
// Sanitize value before setting it
...
a[8]=a[13]=a[18]=a[23]="-";var d=a.join("");return d}}),f.defaults.themes.bootstrap2=f.AbstractTheme.extend({getRangeInput:function(a,b,c){
...
//第五步：设置引入插件对象
f.plugins={ace:{theme:""},epiceditor:{},sceditor:{},select2:{},selectize:{},colorpicker:{},checkboxradio:{},jpicker:{}},
...
//颜色
f.defaults.resolvers.unshift(function(a){if("string"===a.type&&"colorpicker"===a.format)return"colorpicker"}),
//第三步：添加解析器
//jQueryui-checkboxradio
f.defaults.resolvers.unshift(function(a){if("string"===a.type&&"checkboxradio"===a.format)return"checkboxradio"}),
//jpicker
f.defaults.resolvers.unshift(function(a){if("string"===a.type&&"jpicker"===a.format)return"jpicker"}),
